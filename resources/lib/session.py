...
from resources.lib.profiles import get_profile_id, get_account_id, reset_profiles, api_version
...
class Session:
    ...
    def get_token(self):
        ...
        data = api.call_api(url = 'https://http.cms.jyxo.cz/api/' + api_version + '/user.login.step', data = post, sensitive = True)
        ...
        if data['step']['schema'] == 'ShowAccountChooserStep':
            ...
            data = api.call_api(url = 'https://http.cms.jyxo.cz/api/' + api_version + '/user.login.step', data = post)   
        ...
        data = api.call_api(url = 'https://http.cms.jyxo.cz/api/' + api_version + '/user.device.change', data = post, session = self)
        post = {"payload":{"screen":"devices"}}
        data = api.call_api(url = 'https://http.cms.jyxo.cz/api/' + api_version + '/setting.display', data = post, session = self)
        ...
        for device in data['screen']['userDevices']['devices']:
            if device['id'] != deviceId and device['name'] == addon.getSetting('deviceid'):
                ...
                data = api.call_api(url = 'https://http.cms.jyxo.cz/api/' + api_version + '/user.device.remove', data = post, session = self)
        self.save_session()
        ...
        data = api.call_api(url = 'https://http.cms.jyxo.cz/api/' + api_version + '/user.profile.select', data = post, session = self)
